...
This browser coverage is intentionally broader than the deterministic desktop
readability gate used during artifact checks. The artifact checker enforces a
minimum readability baseline at 1440×900; `visual-check` then verifies real
browser containment across the wider desktop viewport set above.
...
Do not treat a passing deterministic readability baseline as evidence that all
desktop viewports are clear. It proves only the minimum artifact gate. The
multi-viewport browser run and perceptual review remain separate claims.

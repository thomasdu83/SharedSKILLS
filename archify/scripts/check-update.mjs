...
function usageText() {
  return [
    ...
    '  --json is accepted for explicit machine invocation.',
  ].join('\n');
}

function normalizeCliArguments(argumentsList) {
  const normalized = [];
...
async function runCli() {
  ...
  const argumentsList = normalizeCliArguments(process.argv.slice(2));

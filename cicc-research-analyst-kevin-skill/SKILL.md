---
name: cicc-research-analyst-kevin-skill
description: Use when the user explicitly asks 刘刚（Kevin） for an analyst perspective, or asks for a CICC perspective on 港股、中概股与海外权益; not for generic explanations or weekly forecast bookkeeping.
metadata:
  openclaw:
    requires:
...
- 点名 刘刚（Kevin） 的一次性咨询由本 Skill 负责；覆盖 港股与中概股；明确点名时也覆盖海外市场和跨资产配置。
...
2. 在完整用户问题后追加“请用刘刚（Kevin）skill回答”，保留原问题的限制。
   在技能根目录运行 `python scripts/get_data.py "Kevin, 怎么看港股？"`；无需留回答文件时加 `--no-save`。

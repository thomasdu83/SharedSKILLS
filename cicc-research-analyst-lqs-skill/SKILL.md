---
name: cicc-research-analyst-lqs-skill
description: Use when the user explicitly asks 李求索 for an analyst perspective, or asks for a CICC perspective on A股市场策略、行业配置与主题投资; not for generic explanations or weekly forecast bookkeeping.
metadata:
  openclaw:
    requires:
...
- 点名 李求索 的一次性咨询由本 Skill 负责；覆盖 A股策略、行业配置、制度和主题投资。
...
2. 在完整用户问题后追加“请用李求索skill回答”，保留原问题的限制。
   在技能根目录运行 `python scripts/get_data.py "李求索, 怎么看A股行业配置？"`；无需留回答文件时加 `--no-save`。

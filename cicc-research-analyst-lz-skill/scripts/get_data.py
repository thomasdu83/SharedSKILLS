...
async def stream_chat_request(query: str, app_id: str, app_secret: str) -> str:
    ...
    error_to_raise = None
    chunk_count = 0
    stream_client = request_cicc_stream_data('POST', API_PATH, params = {}, body = {'query': query + "，请用李昭skill回答", "conversationId": str(uuid.uuid4())}, app_id=app_id, app_secret=app_secret)
...
async def query_chat(
    ...
    try:
        ...
        if save_to_file and content:
            ...
            output_path = out_dir / f"cicc_research_analyst_lz_skill_{unique_suffix}.txt"

---
name: cicc-research-analyst-myl-skill
description: Use when the user explicitly asks 缪延亮 for an analyst perspective, or asks for a CICC perspective on 全球宏观、通胀与国际金融; not for generic explanations or weekly forecast bookkeeping.
metadata:
  openclaw:
    requires:
...
- 点名 缪延亮 的一次性咨询由本 Skill 负责；覆盖 全球宏观、通胀、宏观调控和国际金融。
...
2. 在完整用户问题后追加“请用缪延亮skill回答”，保留原问题的限制。
   在技能根目录运行 `python scripts/get_data.py "缪延亮, 怎么看全球通胀？"`；无需留回答文件时加 `--no-save`。

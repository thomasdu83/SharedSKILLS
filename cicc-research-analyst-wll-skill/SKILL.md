---
name: cicc-research-analyst-wll-skill
description: Use when the user explicitly asks 韦璐璐 for an analyst perspective, or asks for a CICC perspective on 中国债市、宏观利率与机构流动性; not for generic explanations or weekly forecast bookkeeping.
metadata:
  openclaw:
    requires:
...
- 点名 韦璐璐 的一次性咨询由本 Skill 负责；覆盖 利率债、流动性、货币政策、金融监管、资管和机构投资者行为。
...
2. 在完整用户问题后追加“请用韦璐璐skill回答”，保留原问题的限制。
   在技能根目录运行 `python scripts/get_data.py "韦璐璐, 怎么看债市流动性？"`；无需留回答文件时加 `--no-save`。

---
name: cicc-research-analyst-zp-skill
description: Use when the user explicitly asks 周彭 for an analyst perspective, or asks for a CICC perspective on 中国经济制度、资金配置与产业传导; not for generic explanations or weekly forecast bookkeeping.
metadata:
  openclaw:
    requires:
...
- 点名 周彭 的一次性咨询由本 Skill 负责；覆盖 中国经济运行、制度约束、金融周期、产业演化与资产定价传导。
...
2. 在完整用户问题后追加“请用周彭skill回答”，保留原问题的限制。
   在技能根目录运行 `python scripts/get_data.py "周彭, 怎么看产业演化？"`；无需留回答文件时加 `--no-save`。

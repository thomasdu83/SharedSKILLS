...
description: 当已有书面实现计划、需要在没有子代理或不适合子代理分派的情况下顺序执行任务并记录审查检查点时使用；若当前平台可用子代理且任务独立，优先使用 subagent-driven-development。
---
...
**注意：** 这是顺序执行计划的 fallback。若当前平台支持子代理、计划任务基本独立，并且用户没有要求在单线程中执行，请使用 `subagent-driven-development` 而非此技能。
...
4. **运行验证** — 按要求运行测试或检查
5. **记录检查点** — 每完成一个任务记录变更、验证结果和风险；只有在当前目录已验证为 Git 仓库、用户明确要求提交、且 `git status` 确认变更范围清晰时，才执行 `git add` / `git commit`
...
[检查点]
- 修改：src/middleware/validate.js, tests/validation.test.js
- 验证：validation 测试 3/3 通过
- Git：若当前会话已验证仓库状态且用户要求提交，再执行：
...
- 严格按照计划步骤执行
- 不要跳过验证
- 每个任务单独记录检查点；仅在 Git 仓库已验证且用户明确要求时提交，commit message 引用任务编号
- 计划要求时引用相应技能
- 遇到阻塞时停下来，不要猜测
...
**必需的工作流技能：**
- **superpowers:using-git-worktrees** - 当任务需要隔离工作区或用户要求分支/worktree 开发时使用；使用前先验证当前目录是 Git 仓库

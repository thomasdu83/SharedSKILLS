...
当项目是“少量核心指标 + 周期性更新 + HTML 交付”的监控类项目时，默认按下面方式收敛，而不是把它做成轻量回测页或管理后台：

- 先写预注册/方法文档，再写代码；指标名称、定义、方法、数据源、参数、解读口径应由 YAML 或等价配置统一驱动，避免在计算层和报告层双处硬编码。
- 将项目拆成四层：采集层、存储层、计算层、展示层。代理变量、口径修复、字段排除等问题优先落在最靠前且最可复用的一层，而不是在 HTML 层临时兜底。
- 连续时间序列指标与事件频指标分开建模。事件频指标可以进入同一报告，但不应被强行塞进连续趋势监控链路；必要时单独提供事件表、事件统计条和最近一次读数解读。
- CUSUM 只是趋势监控方法之一，可与滚动 z 分数、状态带、分位区间、同比/环比变化、回归系数漂移等方法并列归入“趋势描述/监控方法”层，而不应被写成所有指标监控项目的默认骨架。
- 若原文指标无法完整复现，允许使用代理口径，但必须同时保留三件事：代理映射规则、偏差来源说明、报告中的显式 caveat。
- 面向投研消费的监控项目，首屏卡片不是“展示所有数”，而是让熟悉指标的读者一眼看到：当前值、当前状态、历史位置、样本与截至日期。
...
2. **规划 (Planning)**:
   ...
   - 若任务是指标监控 HTML 项目，规划时额外确认：
     - 哪些指标是连续时序、哪些是事件频
     - 哪些口径来自原文，哪些是代理复现
     - 哪些解释来自统计检测或趋势监控方法，哪些是研究模板解读
     - 报告首屏卡片要让谁“一眼判断状态”，最小必要信息是什么
3. **执行 (Execution)**:
   - 用户确认后执行。
...
当项目输出是静态 HTML 指标监控报告时，默认检查下面几点：

1. 预注册文档中的 `label / definition / method / interpretation / data_source / frequency / params` 能否被代码直接读取并进入报告。
2. 计算层是否明确区分连续趋势监控链路与事件频观察链路，避免把“最近事件读数”误包装成“连续监控报警”。
3. 所有代理口径是否在文档与报告中同步披露，例如节点映射、前向填充、替代变量与缺口说明。
4. 报告层的 section 是否同构：定义、构造方法、解读方法、参数、最新解读、证据图表/表格，避免某个指标因实现方便而退化成临时块。
5. HTML 中的公式、符号和统计口径是否可稳定渲染；若 Python 字符串直接生成 LaTeX，优先使用原始字符串避免转义错误。

...
- `artifacts/` 放单次运行产物、图表、报告、审计证据和项目内快照备份
- 每次正式运行建议创建 `artifacts/research_runs/<run_id>/`，并至少区分 `outputs/`、`audits/`、`diagnostics/`、`logs/`
...
正式量化项目默认拆成三层，层与层之间通过明确 schema 或函数契约连接：

| 层 | 负责内容 | 不应负责 |
|---|---|---|
| 数据快照层 | 取数、原始快照、清洗、字段标准化、可用期记录 | 标签判断、策略阈值 |
| 共享特征层 | 复用型统计量、窗口特征、暴露、持仓画像 | 具体标签名称、最终动作 |
| 标签/信号层 | 标签映射、信号生成、组合动作、风险分桶 | 重复实现底层取数和清洗 |

一个 generator 同时负责取数、特征、标签、审计和导出时，应优先拆分，而不是继续增加分支。

### Point-in-Time 约束

- 每个输入必须明确它相对于 `tradingday` 或 `as_of_date` 的可见性。
- 所有 `merge`、`merge_asof`、`rolling`、`resample` 和标签回填都必须写明时间方向、窗口边界和填充语义。
- 未来信息不能因为缓存、默认最新接口、前向填充或宽松 fallback 被引入。
- 日期精度、时区和自然日/交易日口径必须在入口统一；下游不得自行猜测。
...
3. 必需字段、唯一键和数据版本是否匹配；
4. 缓存是否足够新，是否需要强制刷新；
5. 入口是否真的独立，还是依赖某个未声明的中间表。

只检查“文件存在”不算 preflight。核心标签、信号、风控和 PnL 入口在依赖不满足时必须中断。
...
- 把数据、特征、标签和审计逻辑堆在一个 generator 中
- 用当前最新快照代替目标日期可见的历史快照
- preflight 只检查路径存在，不检查日期覆盖、字段覆盖和依赖状态
- 核心信号缺数据时偷偷补默认值继续生成结果

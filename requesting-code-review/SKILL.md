...
先确认当前目录是 Git 仓库，并查看分支、远端和工作区状态：
...
如果当前目录不是 Git 仓库，或没有可用的 base/head SHA，则使用“文件级审查 fallback”：列出本轮修改文件、关键 diff 摘要、验证命令和结果，不强行走 SHA 流程。
...
当 SHA 范围不可用时，仍然可以审查，但必须明确证据来源：

```
WHAT_WAS_IMPLEMENTED: <本轮完成内容>
PLAN_OR_REQUIREMENTS: <需求或计划>
CHANGED_FILES:
- path/to/file.py: <变更摘要>
- tests/path/test_file.py: <测试摘要>
DIFF_SUMMARY: <关键行为差异，必要时贴小片段>
VERIFICATION:
- <命令> -> <结果>
LIMITATION: 当前目录不是 Git 仓库或没有可用 SHA，未执行 git diff 范围审查。
```

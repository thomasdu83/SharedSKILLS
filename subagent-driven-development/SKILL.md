...
description: 当已有书面实现计划、任务可拆成相对独立的实现单元，并且当前会话可使用子代理执行和审查时使用；若没有实现计划先用 writing-plans，若只是多个独立故障排查用 dispatching-parallel-agents。
---
...
**与 dispatching-parallel-agents 的边界：**
- 本技能用于执行已有实现计划，任务通常会修改代码并需要规格/质量审查。
- `dispatching-parallel-agents` 用于多个独立故障、测试失败或探索性排查，不要求已有计划，也不负责计划级交付。
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "实现子智能体实现、测试、记录检查点、自审" [shape=box];
    ...
    "实现子智能体有疑问?" -> "实现子智能体实现、测试、记录检查点、自审" [label="否"];
    "实现子智能体实现、测试、记录检查点、自审" -> "分派规格审查子智能体 (./spec-reviewer-prompt.md)";
...
[稍后] 实现者：
  ...
  - 已记录检查点；如果当前目录已验证为 Git 仓库且用户要求提交，再提交

[分派规格合规审查]
规格审查者：✅ 符合规格 - 所有需求已满足，无多余内容

[验证 Git/SHA；若不可用则使用文件级审查，分派代码质量审查]

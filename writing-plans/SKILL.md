---
name: writing-plans
description: 当目标和设计方向已经明确、需要把复杂多步骤任务转成可执行实现计划、跨轮次共享计划或多人协作时使用；若需求仍需探索或方案未确认，先用 brainstorming；小改动不需要单独生成计划文档
---
...
为复杂任务编写全面的实现计划，假设工程师对我们的代码库零上下文，且品味存疑。记录他们需要知道的一切：每个任务要修改哪些文件、代码、测试、可能需要查阅的文档、如何测试。将整个计划拆成小步骤任务。DRY。YAGNI。TDD。

假设他们是有经验的开发者，但对我们的工具链和问题领域几乎一无所知。假设他们不太擅长测试设计。

**开始时宣布：** "我正在使用 writing-plans 技能创建实现计划。"

**范围门槛：** 如果只是单文件修复、轻量规则更新、文档措辞调整或其他可直接完成的小改动，不要调用此技能，也不要新建计划 `md`；用 3-5 行内联计划即可。

**上下文：** 当前环境可使用 Git CLI，但每个目录是否为 Git 仓库、是否配置远端、是否适合提交/推送，必须在当前会话中验证；只有在 `git rev-parse --show-toplevel`、`git status --short --branch`、`git remote -v` 已确认且用户明确需要时，才可把 worktree、commit、push 或 PR 写入计划。

**计划保存位置：** 仅在需要持久化书面计划时，使用 `docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md`
- （用户对计划位置的偏好优先于此默认值；小改动使用内联计划）
...
- "实现最少代码让测试通过" - 一步
- "运行测试确认通过" - 一步
- "记录检查点" - 一步（如需 git 提交，再显式写明）

## 计划文档头部

**只有在需要落盘计划文档时，每个计划才以此头部开始：**
...
```bash
记录本步骤已完成；若当前会话已验证 git 可用且用户要求提交，再补充具体 `git add` / `git commit` 命令
```
````
...
- 精确的命令和预期输出
- DRY、YAGNI、TDD
- 仅在 git 能力已验证且用户明确需要时，才把 commit 写成计划步骤
...
如果计划已保存为文档，提供执行选项；如果只是内联计划，直接进入执行即可：

---
name: writing-skills
description: 当需要设计或审查 skill 的触发边界、路由描述、压力测试、CSO 搜索优化或行为有效性时使用；若任务是创建目录、放置 scripts/references/assets、打包或运行脚手架，优先使用 skill-creator。
---
...
本技能负责“技能是否该存在、如何被正确触发、如何验证它真的改变行为”。如果用户要创建目录、生成模板、组织 `scripts/` / `references/` / `assets/`、运行验证或打包命令，使用 `skill-creator` 作为执行层。
